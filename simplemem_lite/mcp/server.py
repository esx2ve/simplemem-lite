"""Thin MCP server for SimpleMem-Lite.

This is a lightweight MCP server that:
1. Reads local files on-demand (traces, code files)
2. Compresses payloads for transport
3. Proxies requests to the backend API
4. Auto-starts local backend if not running (configurable)

The server runs locally but can connect to either a local or cloud backend.
"""
...
async def _get_client() -> BackendClient:
    ...
    If AUTO_START_BACKEND is enabled and the backend is not running,
    it will be started automatically as a subprocess.
    """
    ...
    if _client is None:
        async with _client_lock:
            ...
            if _client is None:
                # Auto-start backend if enabled and not already started
                if AUTO_START_BACKEND and not _backend_started:
                    log.info("Auto-start enabled, ensuring backend is running...")
                    if await ensure_backend_running():
                        _backend_started = True
                        log.info("Backend is ready")
                    else:
                        log.warning(
                            "Failed to auto-start backend, requests may fail. "
                            "Set SIMPLEMEM_AUTO_START=false if using cloud backend."
                        )
...
async def process_trace_batch(
    ...
    """
    try:
        log.info(f"process_trace_batch called ({len(sessions)} sessions)")

        reader = await _get_reader()
        traces_to_process = []
        errors = []

        # Read all trace files locally (offload blocking I/O to thread)
        for session in sessions:
            session_id = session.get("session_id")
            if not session_id:
                errors.append({"session_id": "unknown", "error": "Missing session_id"})
                continue

            trace_content = await asyncio.to_thread(reader.read_trace_file, session_id)
            if trace_content is None:
                errors.append({"session_id": session_id, "error": "File not found"})
                continue
        ...
        if not traces_to_process:
            return {"queued": [], "errors": errors, "job_ids": {}}

        # Send to backend for batch processing
        client = await _get_client()
        result = await client.process_trace_batch(
            traces=traces_to_process,
            max_concurrent=max_concurrent,
        )
        result["errors"].extend(errors)
        return result
    except BackendError as e:
        log.error(f"process_trace_batch failed: {e}")
        return {"queued": [], "errors": [], "job_ids": {}, "error": e.detail}
...
async def discover_sessions(
    ...
    try:
        ...
        # TODO: Check indexed status from backend
        result = {
            "sessions": sessions,
            "total_count": len(sessions),
            "indexed_count": 0,  # Would need backend call to check
...
async def cleanup():
    ...
    if _backend_started:
        try:
            ...
            log.warning(f"Error stopping backend: {e}")
        _backend_started = False
